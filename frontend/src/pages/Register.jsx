      ...
      <div style={styles.card}>
        ...
        <form onSubmit={submit}>
          {['name','email','password'].map(field => (
            <div style={styles.group} key={field}>
              <label style={styles.label}>{field}</label>
              <input style={styles.input} name={field}
        ...
        <p style={styles.link}>
          Have an account? <Link to="/login" style={{ color:'#4ade80' }}>Sign in</Link>
        </p>
      </div>
    </div>
  );
...
const styles = {
  ...
  label: { display:'block', fontSize:'11px', color:'#6b7280', textTransform:'uppercase', letterSpacing:'0.08em', marginBottom:'6px', textTransform:'capitalize' },
